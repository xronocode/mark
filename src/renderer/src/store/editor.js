...
export const useEditorStore = defineStore('editor', {
  ...
  actions: {
    ...
    APPLY_FILE_CHANGE(type, change) {
      ...
      switch (type) {
        ...
        case 'change': {
          ...
          if (autoSave || liveReload || tab.previewMode || tab.watchMode) {
            if (autoSaveTimers.has(id)) {
              clearTimeout(autoSaveTimers.get(id))
              autoSaveTimers.delete(id)
            }
            if (isSaved || liveReload || tab.previewMode || tab.watchMode) {
    ...
    EDIT_ZOOM(zoomFactor) {
      ...
      document.documentElement.style.setProperty('--content-zoom', zoomFactor)
    },
